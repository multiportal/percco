...
function html_iso_contacto(&$name,&$email,&$subject,&$msj){
global $chartset;
 if($chartset=='iso-8859-1'){
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        # Envío de datos
		$title = 'Contacto Web Percco';
        $subject = $_POST["asunto"];
        $name = str_replace(array("\r","\n"),array(" "," ") , strip_tags($_POST["nombre"]));
        $email = filter_var($_POST["email"], FILTER_SANITIZE_EMAIL);
        $tel = $_POST["tel"];
        $msj = $_POST["msj"];
		html_iso_contacto($name,$email,$subject,$msj);
        ...
        if (empty($name) OR !filter_var($email, FILTER_VALIDATE_EMAIL) OR empty($tel) OR empty($subject) OR empty($msj)) {
...
.footer{background-color:#c8102e;color:#fff;font-size:12px;font-weight:bold;text-align:center;padding:6px}
</style>
</head>
<body>
<div>
    <table class="fuente1" width="100%" border="0" cellspacing="0" cellpadding="0">
      ...
      <tr>
        ...
        $headers = "From: CONTACTO PERCCO <$email>". "\r\n";
  		$headers .= 'MIME-Version: 1.0' . "\r\n";
    	$headers .= 'Content-type: text/html; charset=iso-8859-1' . "\r\n";					 

        # Envía el correo.
        $success = @mail($para, $title, $message, $headers);
		//$save=mysqli_query($mysqli,"INSERT INTO ".$DBprefix."contacto (ip,nombre,email,tel,asunto,msj,cat_list,seccion,visible) VALUES ('{$ip}','{$name}','{$email}','{$tel}','{$subject}','{$message}','{$cat_list}','{$sec}','1');") or print mysqli_error($mysqli); 
        if ($success) {
			$save=mysqli_query($mysqli,"INSERT INTO ".$DBprefix."contacto (ip,nombre,email,tel,titulo,asunto,msj,cat_list,seccion,visible) VALUES ('{$ip}','{$name}','{$email}','{$tel}','{$title}','{$subject}','{$message}','{$cat_list}','{$sec}','1');") or print mysqli_error($mysqli); 
