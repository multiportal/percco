...
html_iso_porta($nombre,$des,$resena1,$cate);
	...
	$save=mysqli_query($mysqli,"INSERT INTO ".$DBprefix."galeria (nombre,cover,descripcion,cate,resena,imagen1,imagen2,imagen3,imagen4,imagen5,alta,visible,user,url_page) VALUES ('{$nombre}','{$cover}','{$des}','{$cate}','{$resena}','{$ima1}','{$ima2}','{$ima3}','{$ima4}','{$ima5}','{$alta}','{$visible}','{$username}','{$url_page}')") or print mysqli_error($mysqli);
              ...
              <div class="box-body">
                ...
                <div class="form-group">
                  ...
                  <select class="form-control" id="cate" name="cate">
                    <option value="Poliester" <?php echo $sel1=($cate=='Poliester') ? 'selected' : '';?>>Poliester</option>
                    <option value="Epoxicos" <?php echo $sel1=($cate=='Epoxicos') ? 'selected' : '';?>>Epoxicos</option>
                  </select>                  
                </div>
                <div class="form-group">
...
<script type="text/javascript">
	...
	if($_GET['delete']==1 && !empty($_GET['delete'])){mysqli_query($mysqli,"UPDATE ".$DBprefix."galeria SET ".$c_img."='' WHERE ID='".$_GET['id']."';") or print mysqli_error($mysqli);}
    ?>}
    }
</script>  
<?php 
...
<?php
$sql=mysqli_query($mysqli,"SELECT * FROM ".$DBprefix."galeria WHERE ID='{$id}';") or print mysqli_error($mysqli); 
...
html_iso_porta($nombre,$des,$resena1,$cate);
	...
	$save=mysqli_query($mysqli,"UPDATE ".$DBprefix."galeria SET nombre='{$nombre}', cover='{$cover}', descripcion='{$des}', cate='{$cate}', resena='{$resena}', imagen1='{$ima1}', imagen2='{$ima2}', imagen3='{$ima3}', imagen4='{$ima4}', imagen5='{$ima5}', alta='{$alta}', visible='{$visible}', url_page='{$url_page}' WHERE ID='{$id}';") or print mysqli_error($mysqli);
	validar_aviso($save,'El portafolio se ha guardo correctamente','No se puedo guardar intentelo nuevamente',$aviso);
	$URL=$page_url.'index.php?mod='.$mod.'&ext='.$ext;
	recargar(3,$URL,$target);
	}
...
<script type="text/javascript">
	...
	if($_GET['delete']==1 && !empty($_GET['delete'])){
		mysqli_query($mysqli,"DELETE FROM ".$DBprefix."galeria WHERE id='".$_GET['id']."';") or print mysqli_error($mysqli);
	}
	?>}
    }
</script>  
...
<?php 
$sql=mysqli_query($mysqli,"SELECT * FROM ".$DBprefix."galeria ORDER BY ID ASC;") or print mysqli_error($mysqli); 
...
<div class="col-md-12">
        <?php 
		$query="SELECT * FROM ".$DBprefix."galeria WHERE visible=1 ORDER BY ID ASC;";
